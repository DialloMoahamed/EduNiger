...
import ParentLayout, { useParentAuth } from './ParentLayout';

// ─── API : utilise le token PARENT (parent_token) ──────────────────────────
const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000/api';

function api(path, options = {}) {
  // On lit UNIQUEMENT le parent_token ici
...
export default function ParentMessages() {
  // On utilise useParentAuth() — lit parent_user, PAS useAuth()
  ...
  const handleSend = async (e) => {
    ...
    const d = await api('/messages/send', {
      method: 'POST',
      body: JSON.stringify({
        conversation_id: activeConv.conversation.id,
        contenu: texte.trim(),
      }),
    });
    if (d.success) {
  ...
  return (
    <ParentLayout>
      {/* Conteneur principal — pleine hauteur dans ParentLayout */}
      <div style={{
        display: 'flex',
        height: 'calc(100vh - 120px)', /* topbar (56px) + padding ParentLayout */
        overflow: 'hidden',
        borderRadius: 16,
        ...
        {/* ══ PANNEAU GAUCHE : liste des conversations / contacts ══ */}
        <div style={{
              ...
              {[['convs', 'Conversations'], ['contacts', 'Nouveau']].map(([id, label]) => (
                <button
                  ...
                  style={{
                    ...
                    background: panel === id ? '#0A5C36' : 'var(--bg)',
                    color: panel === id ? '#fff' : 'var(--text-secondary)',
                    cursor: 'pointer',
          ...
          {/* Liste des conversations ou des contacts */}
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {panel === 'convs' ? (
              filteredConvs.length === 0 ? (
                ...
                <div
                  key={c.id}
                  onClick={() => loadConversation(c.autre_id)}
                  style={{
                    display: 'flex', gap: 10,
                    padding: '12px 14px', cursor: 'pointer',
                    background: activeUser?.id === c.autre_id ? '#0A5C3610' : 'transparent',
                    borderLeft: activeUser?.id === c.autre_id
                      ? '3px solid #0A5C36' : '3px solid transparent',
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{
                        fontSize: 13, fontWeight: c.non_lus > 0 ? 600 : 500,
                        color: 'var(--text-primary)',
                    ...
                    <div style={{
                      ...
                      <div style={{
                        fontSize: 12,
                        color: c.non_lus > 0 ? 'var(--text-secondary)' : 'var(--text-muted)',
              ...
              /* Liste des contacts pour démarrer une nouvelle conversation */
              filteredContacts.length === 0 ? (
                ...
                <div
                  key={c.id}
                  onClick={() => { loadConversation(c.id); setPanel('convs'); }}
                  style={{
                    display: 'flex', gap: 10,
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: ROLE_COLORS[c.role] }}>
                      {ROLE_LABELS[c.role]}
                    </div>
                  </div>
                  {c.non_lus > 0 && (
                    <span style={{
                      ...
                      background: '#0A5C36', color: '#fff',
                      fontSize: 10, fontWeight: 600,
                      padding: '1px 6px', borderRadius: 10, alignSelf: 'center',
                    }}>
                      {c.non_lus}
                    </span>
                  )}
        ...
        {activeConv ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              {activeConv.messages.map((m, i) => {
                // On compare avec l'id du parent connecté (user vient de useParentAuth)
                const isMoi = m.sender_id === user?.id;
                const showDate =
                  i === 0 ||
                  formatFull(activeConv.messages[i - 1]?.created_at).split(' ')[0] !==
                  formatFull(m.created_at).split(' ')[0];
                ...
                return (
                  <div key={m.id}>
                    ...
                    <div style={{
                      ...
                      {!isMoi && (
                        <Avatar nom={m.nom} prenom={m.prenom} role={m.role} size={28} />
                      )}
                      <div style={{ maxWidth: '65%' }}>
                        <div style={{
                          padding: '10px 14px',
                          borderRadius: isMoi ? '16px 4px 16px 16px' : '4px 16px 16px 16px',
          ...
          <div style={{
            ...
            <div style={{ fontSize: 13, textAlign: 'center', maxWidth: 280 }}>
              Sélectionnez une conversation ou cliquez sur &quot;Nouveau&quot; pour écrire à un enseignant.
            </div>
          </div>
        )}
      </div>
