  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO notes (eleve_id, matiere_id, classe_id, type_evaluation, note, note_sur, periode, date_evaluation, created_by)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [eleve_id, matiere_id, classe_id, type_evaluation, note, note_sur || 20, periode, date_evaluation, req.user.id]
    );
...
exports.updateNote = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      [eleve_id, matiere_id, classe_id, type_evaluation, note, note_sur || 20, periode, date_evaluation, req.params.id]
    );
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Note non trouvée' });
...
exports.generateBulletin = async (req, res) => {
  try {
    ...
    if (eleves.length === 0) return res.status(404).json({ success: false, message: 'Élève non trouvé' });
    ...
    const [tousEleves] = await db.query(
      'SELECT id FROM eleves WHERE classe_id = ?', [eleve.classe_id]
    );
    const classement = [];
    ...
    const { filename, filepath } = await generateBulletinPDF({
      eleve, ecole, detailsNotes,
      moyenne_generale, rang,
