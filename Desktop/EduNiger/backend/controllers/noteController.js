...
exports.createNote = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO notes (eleve_id, matiere_id, classe_id, type_evaluation, note, note_sur, periode, date_evaluation, created_by)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [
        eleve_id,
        matiere_id,
    ...
    const [newNote] = await db.query("SELECT * FROM notes WHERE id = ?", [
      result.insertId,
    ]);
    res
      .status(201)
      .json({ success: true, message: "Note enregistrée", note: newNote[0] });
  } catch (error) {
  ...
  try {
    ...
    const [result] = await db.query(
      `UPDATE notes SET eleve_id=?, matiere_id=?, classe_id=?, type_evaluation=?,
       note=?, note_sur=?, periode=?, date_evaluation=? WHERE id=?`,
      [
        eleve_id,
        matiere_id,
        classe_id,
    ...
    if (result.affectedRows === 0)
      return res
        .status(404)
        .json({ success: false, message: "Note non trouvée" });
    const [updated] = await db.query("SELECT * FROM notes WHERE id = ?", [
      req.params.id,
    ]);
...
exports.getEleveMoyenne = async (req, res) => {
  try {
    ...
    const byMatiere = {};
    notes.forEach((n) => {
      if (!byMatiere[n.matiere])
        byMatiere[n.matiere] = { notes: [], coef: n.coefficient };
    ...
    const moyenne_generale =
      totalCoef > 0 ? (totalPts / totalCoef).toFixed(2) : 0;
  ...
  try {
    ...
    const [notes] = await db.query(
      `SELECT n.note, n.note_sur, m.nom as matiere, m.coefficient, m.id as matiere_id
       FROM notes n JOIN matieres m ON n.matiere_id = m.id
       WHERE n.eleve_id = ? AND n.periode = ? ORDER BY m.nom`,
      [eleve_id, periode],
    );
    ...
    const [moyClasse] = await db.query(
      `SELECT m.nom as matiere,
              AVG((n.note / n.note_sur) * 20) as moy_classe
       FROM notes n
       JOIN matieres m ON n.matiere_id = m.id
       JOIN eleves e ON n.eleve_id = e.id
       WHERE e.classe_id = ? AND n.periode = ?
       GROUP BY m.id`,
      [eleve.classe_id, periode],
    );
    const moyClasseMap = {};
    moyClasse.forEach((r) => {
      moyClasseMap[r.matiere] = r.moy_classe;
    });
    ...
    const detailsNotes = Object.entries(byMatiere).map(([matiere, d]) => {
      ...
      return {
        ...
        coefficient: d.coef,
        moy_classe: moyClasseMap[matiere]
          ? parseFloat(moyClasseMap[matiere]).toFixed(2)
          : null,
      };
    });

    const moyenne_generale =
      totalCoef > 0 ? (totalPts / totalCoef).toFixed(2) : "0.00";
    ...
    // ── SMS bulletin disponible (fire & forget) ─────────────────
    smsService
      .smsBulletinDisponible({ eleve, moyenne_generale, periode })
      .catch((err) => console.error("SMS bulletin error:", err));

    // Envoyer le PDF directement en streaming (évite la page noire)
