...
exports.login = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: user.id, email: user.email, role: user.role, tenantId: req.tenantId },
...
exports.getProfile = async (req, res) => {
  try {
    const [users] = await db.query(
      'SELECT id, nom, prenom, email, role, telephone, bio, photo, created_at FROM users WHERE id = ? AND tenant_id = ?',
...
exports.changePassword = async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [hashedPassword, req.user.id, req.tenantId]
    );
...
exports.getUsers = async (req, res) => {
  try {
    const [users] = await db.query(
      'SELECT id, nom, prenom, email, role, telephone, created_at FROM users WHERE tenant_id = ? ORDER BY nom, prenom',
      [req.tenantId]
    );
...
exports.register = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO users (tenant_id, nom, prenom, email, password, role, telephone) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [req.tenantId, nom, prenom, email, hashed, role || 'enseignant', telephone || null]
    );
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE users SET nom=?, prenom=?, telephone=?, bio=? WHERE id=? AND tenant_id=?',
      [nom, prenom, telephone || null, bio || null, req.user.id, req.tenantId]
    );
...
exports.uploadPhoto = async (req, res) => {
  try {
    ...
    if (!photo)
      return res.status(400).json({ success: false, message: 'Photo manquante' });

    await db.query(
      'UPDATE users SET photo=? WHERE id=? AND tenant_id=?',
      [photo, req.user.id, req.tenantId]
    );
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const [users] = await db.query(
      'SELECT id, nom, prenom, email FROM users WHERE email = ? AND tenant_id = ?',
      [email, req.tenantId]
    );
    if (users.length === 0)
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:auto;padding:32px;background:#f7f9fc;border-radius:12px">
          ...
          <div style="background:#fff;border-radius:8px;padding:24px;border:1px solid #e2e8f0">
            ...
            <p style="color:#64748b;line-height:1.6">Vous avez demandé la réinitialisation de votre mot de passe.<br>Ce lien est valable <strong>1 heure</strong>.</p>
