...
exports.getClasseById = async (req, res) => {
  try {
    ...
    res.json({ success: true, classe: { ...classes[0], eleves } });
  } catch (error) {
...
exports.createClasse = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO classes (tenant_id, nom, niveau, annee_scolaire, enseignant_id)
       VALUES (?, ?, ?, ?, ?)`,
      [req.tenantId, nom, niveau, annee_scolaire, enseignant_id]
    );

    const [newClasse] = await db.query('SELECT * FROM classes WHERE id = ?', [result.insertId]);
    res.status(201).json({ success: true, message: 'Classe créée avec succès', classe: newClasse[0] });
  } catch (error) {
...
exports.updateClasse = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `UPDATE classes SET nom = ?, niveau = ?, annee_scolaire = ?, enseignant_id = ?
       WHERE id = ? AND tenant_id = ?`,
      [nom, niveau, annee_scolaire, enseignant_id, req.params.id, req.tenantId]
    );

    if (result.affectedRows === 0)
      return res.status(404).json({ success: false, message: 'Classe non trouvée' });

    const [updatedClasse] = await db.query('SELECT * FROM classes WHERE id = ?', [req.params.id]);
    res.json({ success: true, message: 'Classe modifiée avec succès', classe: updatedClasse[0] });
  } catch (error) {
...
exports.deleteClasse = async (req, res) => {
  try {
    const [eleves] = await db.query(
      'SELECT COUNT(*) as count FROM eleves WHERE classe_id = ? AND tenant_id = ?',
      [req.params.id, req.tenantId]
    );

    if (eleves[0].count > 0)
      return res.status(400).json({ success: false, message: 'Impossible de supprimer une classe contenant des élèves' });
